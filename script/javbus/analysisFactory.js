...
module.exports =  class analysisFactory{
  ...
  analysisThumbList(html){
    ...
    var promiseArray = thumbArray.map((detail,idx)=> {
      ...
      return new Promise(resolve=>setTimeout(()=>resolve(),sleepTime))
        ...
        .then(()=>{
              logger.info("抓取详情页延迟",sleepTime,"毫秒,结束");
  ...
  analysisDetail(html,detail){
    ...
    var mvLength = ps[2].children[1].data;
    var mvDirector = $(ps[3]).find("a").text();
    var mvProducers = $(ps[4]).find("a").text();
    var mvPublisher = $(ps[5]).find("a").text();
    var tags = [];
    $(ps[8]).find(".genre").each(function (index,element) {
    ...
    //解析名字
    var mvActorsA = $(ps[10]).find("a");
    var mvActors = [];
    if(mvActorsA){
      mvActorsA.each(function (idx,element) {
    ...
    trs.each(function (idx,element) {
        ...
        var url = $(($(tds[0]).find("a"))[0]).attr("href");
        var size = $(tds[1]).find("a").text() ;
        var time  = $(tds[2]).find("a").text() ;
        mvMagnets.push({url:url,size:size,time:time});
    });
